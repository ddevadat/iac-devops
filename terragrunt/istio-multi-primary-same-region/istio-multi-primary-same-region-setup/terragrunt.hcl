terraform {
  source = "git::${get_env("IAC_TERRAFORM_MODULES_REPO")}//terraform/istio-multi-primary-same-region/istio-multi-primary-same-region-setup?ref=${get_env("IAC_TERRAFORM_MODULES_TAG")}"
}

dependency "istio_infra" {
  config_path = "../istio-multi-primary-same-region-oke-infra"
  mock_outputs = {
